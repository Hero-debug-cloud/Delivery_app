...
export default function CustomersPage() {
  ...
  const handleToggleActive = async (user: User) => {
    ...
    try {
      ...
      console.error(err);
      setPageError(err.message || "Failed to update customer status.");
    }
  };

  const handleDeleteUser = async (user: User) => {
    if (!isSuperAdmin) return;
    if (!window.confirm(`Are you sure you want to delete the customer profile for ${user.name}? This action cannot be undone.`)) {
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setPageError(err.message || "Failed to delete customer profile.");
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-heading-32-bold text-neutral-900 tracking-tight">Customer Management</h1>
          <p className="text-caption-14-regular text-neutral-500 mt-1">View customer profiles, register customer phone records, and manage activation statuses.</p>
        </div>
        {isSuperAdmin && (
          <button 
            onClick={() => {
      ...
      <div className="bg-white border border-neutral-200 rounded-md shadow-card overflow-hidden">
        ...
        <div className="p-4 border-b border-neutral-200 flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 bg-neutral-50">
          ...
          <span className="text-[12px] font-semibold text-neutral-500 self-center">
            {pagination?.totalItems ?? 0} customers registered
          </span>
        </div>
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              ...
              <span className="text-[13px] text-neutral-500 font-semibold">Loading customers database...</span>
            </div>
          ) : users.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              <UsersIcon className="text-neutral-300 mb-3" size={48} />
              <h3 className="text-[15px] font-bold text-neutral-800">No customers found</h3>
